...
`Wtransport.ConnectionHandler` and `Wtransport.StreamHandler` behaviors.

Note that you have to pass the name of the modules of your application that implement the
`Wtransport.ConnectionHandler` and `Wtransport.StreamHandler` behaviors, as options
to the `Wtransport.Supervisor`.

### ConnectionHandler

The `Wtransport.ConnectionHandler` behavior defines the interface that Wtransport uses to pass `Wtransport.Connection`s up to the application level;
...
The `Wtransport.StreamHandler` behavior defines the interface that Wtransport uses to pass `Wtransport.Stream`s up to the application level;
it is used to handle:
